...
--게시판리스트 + comment
select * ,
from board b left outer join boardcomment bc on  b.boardno=bc.boardno order by groupno desc,step asc ;

select boardno,title,groupno,step,depth ,(select count(*) cnt from boardcomment where boardno=b.boardno) 
from board b order by groupno desc,step asc;
